...
import {
  Expense, Income, Budget, Category, Card,
  Saving, SavingTransaction, SavingTransactionType,
  FixedTerm, Investment,
...
export const createExpense = async (e: Omit<Expense, 'id'>): Promise<Expense> => {
  const uid = getUid();
  const ref = await addDoc(collection(db, 'users', uid, 'expenses'), cleanUndefined(e));
  return { id: ref.id, ...e };
};
...
export const createIncome = async (i: Omit<Income, 'id'>): Promise<Income> => {
  const uid = getUid();
  const ref = await addDoc(collection(db, 'users', uid, 'incomes'), cleanUndefined(i));
  return { id: ref.id, ...i };
};
...
export const createBudget = async (b: Omit<Budget, 'id'>): Promise<Budget> => {
  const uid = getUid();
  const ref = await addDoc(collection(db, 'users', uid, 'budgets'), cleanUndefined(b));
  return { id: ref.id, ...b };
};
...
export const createCategory = async (c: Omit<Category, 'id'>): Promise<Category> => {
  const uid = getUid();
  const ref = await addDoc(collection(db, 'users', uid, 'categories'), cleanUndefined(c));
  return { id: ref.id, ...c };
};
...
export const checkAndSeedCategories = async (): Promise<boolean> => {
  ...
  const { ALL_DEFAULT_CATEGORIES } = await import('@controlados/shared');
  await Promise.all(
...
export const updateMonthlyIncome = async (amount: number): Promise<number> => {
  const uid = getUid();
  await setDoc(doc(db, 'users', uid, 'config', 'monthlyIncome'), { amount });
  return amount;
};
...
export const createCard = async (c: Omit<Card, 'id'>): Promise<Card> => {
  const uid = getUid();
  const ref = await addDoc(collection(db, 'users', uid, 'cards'), cleanUndefined(c));
  return { id: ref.id, ...c };
};
...
export const createSavingTransaction = async (
  t: Omit<SavingTransaction, 'id'>
): Promise<{ transaction: SavingTransaction; newBalance: number }> => {
  const uid = getUid();
